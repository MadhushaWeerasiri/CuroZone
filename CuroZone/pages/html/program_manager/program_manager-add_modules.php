...
?>
	...
	<body>
		...
		<div class="side-bar">
			...
			<div class="menu">
				...
				<div class="item"><a class="sub-btn" id="active-item"><i class="fas fa-server"></i>Courses
					...
					<div class="sub-menu">
						<a id="active-item" class="sub-item">Add Courses</a>
		...
		<div class="mainframe">
			<div class="welcome">
				<h2 style="text-align: center; padding: 5px;">Add Modules</h2>
			...
			<div class="task">
				<?php
				$sql = "SELECT * FROM course_tbl WHERE course_id = '$_POST[courseidtxt]'";
				$result = mysqli_query($con, $sql);
				if (mysqli_num_rows($result) == 0) {
					$_SESSION['course_id'] = $_POST['courseidtxt'];
					//echo "<script> console.log('$_SESSION[course_id]') </script>";
					$_SESSION['course_name'] = $_POST['coursenametxt'];
					//echo "<script> console.log('$_SESSION[course_name]') </script>";
					$_SESSION['department'] = $_POST['departmentselect'];
					//echo "<script> console.log('$_SESSION[department]') </script>";
					$_SESSION['duration'] = $_POST['durationtxt'];
					//echo "<script> console.log('$_SESSION[duration]') </script>";
					$_SESSION['start_date'] = $_POST['startdatetxt'];
					//echo "<script> console.log('$_SESSION[start_date]') </script>";
					$_SESSION['modulenum'] = $_POST['modulenumber'];
					// "<script> console.log('$_SESSION[modulenum]') </script>";
				?>
					<form id="form1" name="form1" method="post" action="program_manager-add_courses_check.php">
						<table id="table" width="60%">
							<tbody>
								<?php
								for ($x = 1; $x <= $_POST['modulenumber']; $x++) {
								?>
									<tr>
										<td rowspan="3" width="35%" style="text-align: right"><label for="textfield">Module <?php echo $x; ?></label></td>
										<td width="8%" style="text-align: center">:</td>
										<td width="57%" style="text-align: left"><input type="text" name="<?php echo "moduleidtxt+$x"; ?>" id="<?php echo "moduleidtxt+$x"; ?>" placeholder="Module <?php echo $x; ?> ID" required></td>
									</tr>
									<tr>
										<td style="text-align: center">:</td>
										<td style="text-align: left"><input type="text" name="<?php echo "modulenametxt+$x"; ?>" id="<?php echo "modulenametxt+$x"; ?>" placeholder="Module <?php echo $x; ?> Name" required></td>
									</tr>
									<tr>
										<td style="text-align: center">:</td>
										<td style="text-align: left"><select name="<?php echo "semestertxt+$x"; ?>" id="<?php echo "semestertxt+$x"; ?>" required>
												<option value="" selected disabled>Module <?php echo $x; ?> Semester</option>
								...
								?>
								<tr>
									<td colspan="3" style="text-align: center"><input type="submit" name="submit" id="submit" value="Next >"></td>
								</tr>
							</tbody>
						</table>
					</form>
				...
				<?php
					...
					header("Location: /pages/html/program_manager/program_manager-courses_exists.php");
				}
				?>
			</div>
		</div>
